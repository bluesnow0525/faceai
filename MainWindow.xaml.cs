...
namespace FacePractice
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private void BtnCreate(object sender, RoutedEventArgs e)
        {
            if (souce_box.SelectedItem != null && !personName.Text.Equals(""))
            {
                ...
                if (t.Result)
                {
                    ...
                    {
                        case 0:
                            foreach (string url in TraningSource.iuFaces)
                            {
                            ...
                            break;
                        case 1:
                            foreach (string url in TraningSource.bearFaces)
                            {
                                Task<bool>.Run(() => apiAct.AddFace(personID, url));
                                Thread.Sleep(5000);
                            }
        ...
        private void BtnIdentify(object sender, RoutedEventArgs e)
        {
            if (IdentifyUrl.Text != null && !IdentifyUrl.Text.Equals(""))
            {
                APIAction apiAct1 = new APIAction();
                Task<List<PersonObj>> tForListAllPersons = Task<List<PersonObj>>.Run(() => apiAct1.ListAllPersons());
                if (tForListAllPersons.Result.Count != 0)
                {
                    string imgUrl = IdentifyUrl.Text;
                ...
                {
                    System.Windows.MessageBox.Show("no person to identify");
                }
            }
        }
        ...
        private void BtnDetect(object sender, RoutedEventArgs e)
        {
            if (IdentifyUrl.Text != null && !IdentifyUrl.Text.Equals(""))
            {
                IdentifyView.ItemsSource = new List<string>();
